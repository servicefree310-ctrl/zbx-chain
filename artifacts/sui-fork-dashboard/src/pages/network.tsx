    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        ...
        <p className="text-lg text-muted-foreground">
          Topology, port requirements, and full-node setup for external access.
        </p>
      </div>

      <div className="grid md:grid-cols-2 gap-6">
        <div className="border border-border p-5 rounded-lg bg-card">
          <h3 className="font-semibold text-lg text-primary mb-2">Validator Node</h3>
          <ul className="text-sm text-muted-foreground space-y-2 list-disc pl-4">
            <li>Participates in consensus</li>
            <li>Executes all transactions</li>
            <li>Requires high hardware specs</li>
            <li>Should <strong className="text-foreground">not</strong> expose RPC publicly</li>
          </ul>
        </div>
        <div className="border border-border p-5 rounded-lg bg-card">
          <h3 className="font-semibold text-lg text-primary mb-2">Full Node</h3>
          <ul className="text-sm text-muted-foreground space-y-2 list-disc pl-4">
            <li>Follows network state</li>
          ...
          <Table>
            ...
            <TableBody>
              <TableRow className="border-b border-border hover:bg-muted/30">
                ...
                <TableCell>P2P Networking</TableCell>
      ...
      <div className="space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          A standard configuration for a public-facing RPC node (fullnode.yaml).
        </p>
        <CodeBlock 
          language="yaml"
          code={`---
...
p2p-config:
  seed-peers:
    - address: "/ip4/<VALIDATOR_IP>/tcp/8080/http"
      peer-id: "<VALIDATOR_PEER_ID>"
`}
        />
      </div>
    </div>
